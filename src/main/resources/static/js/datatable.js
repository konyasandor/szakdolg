$(function(){
    ...
    //*******************************************************************vevő mérlegjegy table *********************************************//
    
    
    $(document).ready( function () {
        $('#vevoMerlegjegyTable').DataTable({
        	"sAjaxSource": "/termelok",
			"sAjaxDataProp": "",
			"order": [[ 0, "asc" ]],
			"aoColumns": [
				  ...
				  { "mData": "termeloCim" },
				  { "mData": "termeloTelefonSzam" },
				  { "defaultContent": '<button type="button" class="btn newVevoMerleg"> <i class="fas fa-external-link-alt"></i></button>' }
	...
	$(document).ready(function() {
	    var table = $('#merlegjegyLista').DataTable( {
	        ...
	        "columns": [
	            ...
	            { "data": "szamlaSorszam" },
	            { "data": "osszAr" },
	            { "defaultContent": '<button type="button" class="btn editMerlegjegy"> <i class="far fa-edit"></i></button>' },
				{ "defaultContent": '<button type="button" class="btn deleteMerlegjegy"> <i class="far fa-trash-alt"></i></button>' }
	        ],
	        "order": [[1, 'asc']]
	    } );
	...
	//*********************************************************************Vevő mérlegjegy lista *************************************************************************//
	
	    $(document).ready( function () {
        $('#vevomerlegjegyLista').DataTable({ 
			 "ajax": {"url":"/merleg/getvevo",
	        		"dataSrc": ""},
	        "columns": [
			      	{ "data": "id" },
		            { "data": "tNev" },
		            ...
		            { "data": "fizetve", 
		            	render: function(data){
		            		let font = '';
		            		if(!data){
		            			font = '<i class="fas fa-times-circle"></i>';
		            		}
		            		else{
		            			font = '<i class="fas fa-check-circle"></i>';
				  	...
				  	{ "defaultContent": '<button type="button" class="btn modifyvevomjegy"> <i class="fas fa-external-link-alt"></i></button>' }
	...
	$(document).ready(function() {
	    var table = $('#felvasarlasiJegyFizetve').DataTable( {
	        "ajax": {"url":"/felvasarlasjegytablelista",
	        		"dataSrc": ""},
	        "columns": [
	            {
	                "className":      'details-control',
	            ...
	            { "data": "id" },
	            { "data": "tNev" },
	            { "data": "felvasarlasiJegyKeszitese" },
	            { "data": "osszErtek" }
	        ],
	        "order": [[1, 'asc']]
	    } );
	    ...
	    // Add event listener for opening and closing details
	    $('#felvasarlasiJegyFizetve tbody').on('click', 'td.details-control', function () {
	        var tr = $(this).closest('tr');
	        var row = table.row( tr );
	        ...
	        if ( row.child.isShown() ) {
	            ...
	            // Open this row
	            row.child( formatFv(row.data()) ).show();
	            tr.addClass('shown');
	        }
	    } );
	} );
	
	 //*********************************************************************merlegjegy unpaid table ***********************************************************************//
	...
	$(document).ready(function() {
	    var table = $('#UnpaidMerlegjegyLista').DataTable( {
	        "ajax": {"url":"/merlegjegytablelista",
	        		"dataSrc": ""},
	        "columns": [
	            {
	            ...
	            { "data": "leadasDatum" },
	            { "data": "osszAr" },
	            { "defaultContent": '<button type="button" class="btn addJegy text-center"> <i class="far fa-plus-square"></i></button>' }
	    ...
	    // Add event listener for opening and closing details
	    $('#UnpaidMerlegjegyLista tbody').on('click', 'td.details-control', function () {
	        var tr = $(this).closest('tr');
	        var row = table.row( tr );
